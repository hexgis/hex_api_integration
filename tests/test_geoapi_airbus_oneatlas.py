...
def image_id():
    ...
    Note: It's an image that covers Brazilia, if its not available in
    a few years, keep the zoom level, x and y data and replace 
    image id with a valid image.
    """
    return "3e22f256-8898-4b65-b8f1-b15dc1b41aaf"


@pytest.fixture
def zxy_path():
    """ZXY image path for image_id 3e22f256-8898-4b65-b8f1-b15dc1b41aaf.
...
def test_get_oneatlas_url(geo_api):
    ...
    oneatlas_url = geo_api.get_opensearch_api_url()
    assert oneatlas_url
...
def test_get_payload_data_parameter(geo_api):
    ...
    assert payload
    assert type(payload) == dict
    assert payload.get('cloudCover') == '100.0['
...
def test_get_response_data_image_path_feature(geo_api, bbox):
    """Gets response image path for GeoAPI with path from feature."""
...
def test_get_response_for_filtered_data(geo_api, bbox):
    ...
    args = {
        'bbox': bbox,
        'acquisition_date_range': ['2019-01-01', '2020-02-01'],
        'constellation': ['PHR'],
        'cloud_cover': 10,
    }
